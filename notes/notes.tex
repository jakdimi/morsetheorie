...
\begin{lemma}[compactly supported vectorfields generate 1-parameter groups]
   ...
   A compactly supported vectorfield generates a unique 1-parameter group of 
...
\begin{proof}
   ...
   Let Now $X$ be a compactly supported vector field. Assume there exists a 
   1-parameter group of diffeomorphisms $\varphi_t$ that is generated by $X$.
   Then the path $t \mapsto \varphi_t(p)$ for some fixed $p \in M$ satisfies the
   ...
   $U$ of p which is a subset of the support of $X$, such that $\psi(U)$ is 
   bounded. Then in local coordinates we get
...
\begin{proof}
   ...
   lemma~\ref{lemma:vectorfields generate 1-parameter groups}, hence X generates 
...
\begin{theorem}[Second deformation Lemma]
   ...
   $\epsilon > 0$, s.th. $f^{-1}[c-\epsilon, c+\epsilon]$ is compact and 
   contains no critical points of $f$. Then $M^{c+\epsilon}$ is obtained from 
   $M^{c-\epsilon}$ by attaching a $k$-cell.
\end{theorem}
