...
\begin{figure}[H]
   \centering
   \begin{tikzcd}
      S^{k-1} \arrow[r] \arrow[d] \arrow[dr, phantom, "\ulcorner", very near end] 
         & M \arrow[d] \\
      D^k \arrow[r] 
         & X
   \end{tikzcd}
...
\begin{definition}[smooth deformation retract]
   ...
   \[ r(\cdot,0) = \text{id}_M \text{ , } f(M, 1) \subseteq A \text{ , } f(\cdot, 1)|_A = \text{id}_A\]
...
\begin{proof}
   ...
   $u^1, ..., u^n$ in a neighborhood of $p$ such that 
