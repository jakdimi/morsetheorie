...
steigt parallel zum "Boden" via eines Diffeomorphismus $\varphi$ "nach oben zu 
ziehen", bis $\varphi(f^{-1}(a)) = f^{-1}(b)$.
...
\begin{lemma}
    ...
    Es sei $X$ ein Vektorfels auf einer glatten Mannigfaltigkeit $M$, sodass
...
\begin{proof}[Erstes Deformations-Lemma]
    ...
    Für $p \in M$, $t_0 \in \R$ und $q = \varphi^p(t_0)$:
    \begin{align*}
        \left. \frac{d}{dt} f \circ \varphi^p \right|_{t_0}
    ...
    Dann wird unter $\varphi_{b-a}$ das Level-Set $f^{-1}(a)$ auf das Level-Set
    $f^{-1}(b)$ abgebildet, denn für $p \in f^{-1}(a)$ gilt:
    ...
    Wir zeigen nun die folgende Behauptung für 
    $p, q, \varphi_t(p), \varphi_t(q) \in \supp(X)$:
